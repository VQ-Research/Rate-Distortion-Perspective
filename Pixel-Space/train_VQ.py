...
def main_worker(args):
    ...
    codebook = list(vq_model.quantizer.embedding.parameters())
    if args.VQ == "wasserstein_vq" or args.VQ == "mmd_vq":
        ...
        optimizer = torch.optim.AdamW([{'params': model_para}, {'params': code_para, 'lr': 0.01}], lr=args.lr, betas=(0.9, 0.95), weight_decay=0.00001)
    elif args.VQ == "vanilla_vq" or args.VQ == "online_vq":
        model_para = codebook + list(vq_model.projector_out.parameters()) + list(vq_model.projector_in.parameters()) 
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        for step, (x, _) in enumerate(train_dataloader):
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float32):
                ...
                optimizer.zero_grad()
                loss, rec_loss, p_loss, utilization, perplexity = vq_model(x)
