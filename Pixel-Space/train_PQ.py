...
def main_worker(args):
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        if epoch % args.eval_epochs == 0 and int(os.environ['LOCAL_RANK']) == 0:
            ...
            save_checkpoint({'epoch': epoch, 'model': pq_model.module.state_dict(), 'optimizer': optimizer.state_dict(), 'args': args}, is_best=False, filename=checkpoint_path) 
            eval_reconstruction_epoch(args, pq_model, epoch)

        if epoch % args.eval_epochs == 0:
            with torch.no_grad():
                results_pack = eval_one_epoch_pq(args, pq_model, epoch, val_dataloader, len_val_set)
    ...
    if int(os.environ['LOCAL_RANK']) == 0:
        ...
        save_checkpoint({'epoch': epoch, 'model': pq_model.module.state_dict(), 'optimizer': optimizer.state_dict(), 'args': args}, is_best=False, filename=checkpoint_path) 
        eval_reconstruction(args, pq_model)
    pq_model.eval() 
