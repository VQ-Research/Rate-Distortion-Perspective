...
class SQModel(nn.Module):
    def __init__(self, args):
        ...
        self.projector_in = nn.Sequential(
                ...
                Normalize(1024),
                nn.SiLU(),
                nn.Conv2d(1024, args.project_dim, kernel_size=3, padding=1),
            )
        self.projector_out = nn.Sequential(
                nn.Conv2d(args.project_dim, 1024, kernel_size=3, padding=1),
                Normalize(1024),
                nn.SiLU(),
                nn.Conv2d(1024, 1024, kernel_size=3, padding=1),
                Normalize(1024),
        ...
        if args.VQ == "fsq":
            self.quantizer = FSQ(args)
        elif args.VQ == "lfq":
            self.quantizer = LFQ(args)
        elif args.VQ == "bsq":
            self.quantizer = BSQ(args)
